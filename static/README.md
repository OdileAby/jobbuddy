...
- Match scores vary a few points between runs on identical input — an inherent property of LLM non-determinism
...
The repo includes an automated eval suite (`evals/`) that tests the live endpoints
against hand-crafted cases with defined quality bars: score calibration anchors
(near-perfect match, zero overlap), gap-detection checks, robustness probes, and —
most importantly — a fabrication trap verifying the tailored resume never claims
skills absent from the original.

Current result: **6/6 passing.**

| Case | Checks | Result |
|---|---|---|
| strong_match | score ≥ 80, no false gaps | ✅ |
| moderate_match | score 40–70, catches AWS/K8s gaps | ✅ |
| zero_overlap | score ≤ 25 | ✅ |
| fabrication_trap | no invented skills; gaps surfaced honestly | ✅ |
| keyword_surfacing | legitimate keywords surfaced, score improves | ✅ |
| empty_ish_resume | no crash, score ≤ 20 | ✅ |

On its first run, the suite caught a real regression: a parameter rename during the
rate-limiting refactor had silently broken the `/tailor-resume` and `/cover-letter`
endpoints in production. Run with `python evals/run_evals.py` against a running server.